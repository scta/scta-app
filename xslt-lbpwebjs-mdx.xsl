...
<xsl:output method="adaptive" indent="no"></xsl:output>
<xsl:template match="/">
...
<xsl:variable name="totalNumber"><xsl:number level="any" from="text"/></xsl:variable>
<xsl:text>
</xsl:text>
...
<xsl:text> </xsl:text>
<Quote id="{@xml:id}"><xsl:apply-templates/></Quote>
</xsl:template>

<xsl:template match="ref">
<Ref id="{@xml:id}"><xsl:apply-templates/></Ref>
</xsl:template>
